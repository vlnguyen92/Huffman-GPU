...
#include "vlc_kernel_sm3_huff.cu"
...
void runVLCTest(char *file_name, uint num_block_threads, uint num_blocks) {
	...
	CUT_SAFE_CALL(cutStartTimer(timer));
	for (int i=0; i<NT; i++) {
	vlc_encode_kernel_gm0<<<grid_size, block_size, sm_size>>>(d_sourceData, d_codewords, d_codewordlens,  
																					#ifdef TESTING
	...
	CUT_SAFE_CALL(cutStopTimer(timer));
	ktime += cutGetTimerValue(timer);
	printf("GPU Encoding time (GM0): %f (ms)\n", ktime/NT);
	//////////////////* END KERNEL *///////////////////////////////////
#endif
	...
	CUT_SAFE_CALL(cutStartTimer(timer));
	for (int i=0; i<NT; i++) {
	vlc_encode_kernel_gm1<<<grid_size, block_size, sm_size>>>(d_sourceData, d_codewords, d_codewordlens,  
																					#ifdef TESTING
																					d_cw32, d_cw32len, d_cw32idx, 
																					#endif
																d_destData, d_cindex); //testedOK2
	...
	CUT_SAFE_CALL(cutStartTimer(timer));
	for (int i=0; i<NT; i++) {
	vlc_encode_kernel_gm2<<<grid_size, block_size, sm_size>>>(d_sourceData, d_codewords, d_codewordlens,  
																					#ifdef TESTING
																					d_cw32, d_cw32len, d_cw32idx, 
																					#endif
																d_destData, d_cindex); //testedOK2
    ...
    grid_size.x		= num_blocks;
    block_size.x	= num_block_threads;
	sm_size			= 2*block_size.x*sizeof(unsigned int);
	ktime			= 0;
	CUT_SAFE_CALL(cutResetTimer(timer));
	CUT_SAFE_CALL(cutStartTimer(timer));
	for (int i=0; i<NT; i++) {
	vlc_encode_kernel_sm1<<<grid_size, block_size, sm_size>>>(d_sourceData, d_codewords, d_codewordlens,  
																					#ifdef TESTING
																					d_cw32, d_cw32len, d_cw32idx, 
																					#endif
																d_destData, d_cindex); //testedOK2
	...
	CUT_SAFE_CALL(cutStartTimer(timer));
	for (int i=0; i<NT; i++) {
	vlc_encode_kernel_sm2<<<grid_size, block_size, sm_size>>>(d_sourceData, d_codewords, d_codewordlens,  
																					#ifdef TESTING
																					d_cw32, d_cw32len, d_cw32idx, 
																					#endif
																d_destData, d_cindex); //testedOK2
	...
	CUT_SAFE_CALL(cutStartTimer(timer));
	for (int i=0; i<NT; i++) {
	vlc_encode_kernel_smb<<<grid_size, block_size, sm_size>>>(d_sourceData, d_codewords, d_codewordlens,  
																					#ifdef TESTING
																					d_cw32, d_cw32len, d_cw32idx, 
																					#endif
																d_destData, d_cindex); //testedOK2
	...
	CUT_SAFE_CALL(cutStopTimer(timer));
	ktime += cutGetTimerValue(timer);
	printf("GPU Encoding time (SM BASIC): %f (ms)\n", ktime/NT);
	//////////////////* END KERNEL *///////////////////////////////////
#endif
	...
	CUT_SAFE_CALL(cutStartTimer(timer));
	for (int i=0; i<NT; i++) {
	vlc_encode_kernel_sm2_huff<<<grid_size, block_size, sm_size>>>(d_sourceData, d_codewords, d_codewordlens,  
																					#ifdef TESTING
																					d_cw32, d_cw32len, d_cw32idx, 
																					#endif
																d_destData, d_cindex); //testedOK2
	...
	CUT_SAFE_CALL(cutStopTimer(timer));
	ktime += cutGetTimerValue(timer);
	printf("GPU Encoding time (SM2H): %f (ms)\n", ktime/NT);
	//////////////////* END KERNEL *///////////////////////////////////
#endif
	...
	CUT_SAFE_CALL(cutStopTimer(timer));
	ktime += cutGetTimerValue(timer);
	printf("GPU Encoding time (SM3HUFF): %f (ms)\n", ktime/NT);
	//////////////////* END KERNEL *///////////////////////////////////
#endif
	...
	CUT_SAFE_CALL(cutStartTimer(timer));
	for (int i=0; i<NT; i++) {
	////vlc_encode_kernel_dpt<<<num_blocks, ENCODE_THREADS, (ENCODE_THREADS+2*NUM_SYMBOLS)*sizeof(unsigned int)>>>
	//vlc_encode_kernel_dpt<<<num_blocks, num_block_threads/DPT, (2*NUM_SYMBOLS + num_block_threads/DPT + num_block_threads)*sizeof(unsigned int)>>>
	...
	block_size.x	= num_block_threads;
	grid_size.x		= num_blocks/DPT;
	sm_size			= 2*NUM_SYMBOLS*sizeof(int) + block_size.x*sizeof(unsigned int)+ 2*block_size.x*DPT*sizeof(unsigned int);
	if (sm_size>= MAX_SM_BLOCK_SIZE_GPU) {		printf("Ivalid kernel configuration: Reduce SM requirements!\n");	}
	else if (block_size.x>256)			 {		printf("Ivalid kernel configuration: Reduce number of threads/block!\n");	}
	else if (grid_size.x>32*1024)		 {		printf("Ivalid kernel configuration: Reduce number of blocks!\n");	}
	else {
	...
	CUT_SAFE_CALL(cutStartTimer(timer));
	for (int i=0; i<NT; i++) {
	vlc_encode_kernel_dpt2b<<<grid_size, block_size, sm_size>>>(d_sourceData, d_codewords, d_codewordlens,  
																					#ifdef TESTING
																					d_cw32, d_cw32len, d_cw32idx, 
																					#endif
																d_destData, d_cindex);//testedOK

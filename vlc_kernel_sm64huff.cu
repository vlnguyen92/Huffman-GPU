...
#ifndef _VLC_SM256HUFF_KERNEL_H_
#define _VLC_SM256HUFF_KERNEL_H_
...
__global__ static void vlc_encode_kernel_sm64huff(unsigned int* data,
        const uint256* gm_codewords, const unsigned int* gm_codewordlens,
    ...
    //#ifdef CACHECWLUT
#if 0
    uint256* codewords		= (unsigned int*) sm; 
    ...
    for(unsigned int i=0; i<4;i++) {
        tmpbyte = (unsigned char)(val32>>((3-i)*8));
        num = gm_codewords[tmpbyte];
        tmpcw32 = num.operator=(num);
    ...
    uint256 p1 = (cw256>>(codewordlen - wrbits));
    uint256 one = 1;
    tmpcw32		= p1.operator=(p1);		
    ...
    if (codewordlen) {
        ...
        uint256 p2 = (cw256>>(codewordlen - wrbits)) & ((one<<wrbits)-1);
        tmpcw32		= p2.operator=(p2);	
    ...
    /*Part 3*/
    if (codewordlen) {
        uint256 p3 = (cw256 & ((one<<codewordlen)-1));
        tmpcw32		= p3.operator=(p3);

...
//    printf("CPU Encoding time (CPU): %f (ms)\n", ktime);
    ...
    {
        //        cout << i << endl;
        CUDA_SAFE_CALL(cudaMemcpy(d_sourceData,		sourceData,		partSize,		cudaMemcpyHostToDevice));
        ...
        CUDA_SAFE_CALL(cudaMemcpy(d_destData,		destData,		partSize,		cudaMemcpyHostToDevice));
#if 1
        //////////////////* SM64HUFF KERNEL *///////////////////////////////////
        ...
        deallocBlockSums();
        CUDA_SAFE_CALL(cudaMemcpy(destData, 
                    //partialDestData, 
                    d_destDataPacked, 
                    partSize, 
...
//    compare_vectors((unsigned int*)crefData, (unsigned int*)destData, num_ints);
    ...
    free(sourceData); free(destData);  	free(codewords);  	free(codewordlens); free(crefData); 

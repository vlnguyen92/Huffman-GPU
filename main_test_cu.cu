...
void runVLCTest(char *file_name, uint num_block_threads, uint num_blocks) {
    ...
    uint *destData;
    cudaHostAlloc((void**)&destData,mem_size,cudaHostAllocDefault);
...
//    uint	*destData   =	(uint*) malloc(mem_size);
    ...
    memset(codewords,    0, NUM_SYMBOLS*sizeof(uint256));
    ...
    unsigned int	*d_sourceData0, *d_destData0, *d_destDataPacked0;
    unsigned int	*d_sourceData1, *d_destData1, *d_destDataPacked1;
    uint256	*d_codewords; 
    unsigned int  *d_codewordlens;
    unsigned int	 *d_cindex0, *d_cindex2_0;
    unsigned int	 *d_cindex1, *d_cindex2_1;
...
//    ktime = 0;
    ...
    CUDA_SAFE_CALL(cudaMalloc((void**) &d_sourceData0,		  partSize));
    CUDA_SAFE_CALL(cudaMalloc((void**) &d_destData0,			  partSize));
    CUDA_SAFE_CALL(cudaMalloc((void**) &d_destDataPacked0,	  partSize));

    CUDA_SAFE_CALL(cudaMalloc((void**) &d_sourceData1,		  partSize));
    CUDA_SAFE_CALL(cudaMalloc((void**) &d_destData1,			  partSize));
    CUDA_SAFE_CALL(cudaMalloc((void**) &d_destDataPacked1,	  partSize));
    ...
    CUDA_SAFE_CALL(cudaMalloc((void**)&d_cindex0,         new_blocks*sizeof(unsigned int)));
    CUDA_SAFE_CALL(cudaMalloc((void**)&d_cindex2_0,        new_blocks*sizeof(unsigned int)));

    CUDA_SAFE_CALL(cudaMalloc((void**)&d_cindex1,         new_blocks*sizeof(unsigned int)));
    CUDA_SAFE_CALL(cudaMalloc((void**)&d_cindex2_1,        new_blocks*sizeof(unsigned int)));
    ...
    CUDA_SAFE_CALL(cudaMemcpy(d_codewords,		codewords,		NUM_SYMBOLS*sizeof(uint256),	cudaMemcpyHostToDevice));
    CUDA_SAFE_CALL(cudaMemcpy(d_codewordlens,	codewordlens,	NUM_SYMBOLS*symbol_type_size,	cudaMemcpyHostToDevice));
    ( cudaEventRecord( start, 0 ) );

    for(int i = 0; i < num_elements; i+=new_num_elements*2)    
    {
        //        cout << i << endl;
//        CUDA_SAFE_CALL(cudaMemcpy(d_sourceData0,		sourceData+i,		partSize,		cudaMemcpyHostToDevice));
//        CUDA_SAFE_CALL(cudaMemcpy(d_sourceData1,		sourceData+i+new_num_elements,		partSize,		cudaMemcpyHostToDevice));
//        CUDA_SAFE_CALL(cudaMemcpy(d_destData0,		destData+i,		partSize,		cudaMemcpyHostToDevice));
//        CUDA_SAFE_CALL(cudaMemcpy(d_destData1,		destData+i+new_num_elements,		partSize,		cudaMemcpyHostToDevice));

        CUDA_SAFE_CALL(cudaMemcpyAsync(d_sourceData0,		sourceData+i,		partSize,		cudaMemcpyHostToDevice, stream0));
        CUDA_SAFE_CALL(cudaMemcpyAsync(d_sourceData1,		sourceData+i+new_num_elements,		partSize,		cudaMemcpyHostToDevice,stream1));
        CUDA_SAFE_CALL(cudaMemcpyAsync(d_destData0,		destData+i,		partSize,		cudaMemcpyHostToDevice,stream0));
        CUDA_SAFE_CALL(cudaMemcpyAsync(d_destData1,		destData+i+new_num_elements,		partSize,		cudaMemcpyHostToDevice,stream1));
#if 1
        //////////////////* SM64HUFF KERNEL *///////////////////////////////////
...
//        ktime			= 0;
        ...
        //	CUT_SAFE_CALL(cutStartTimer(timer));
        //        for (int i=0; i<NT; i++) {
        vlc_encode_kernel_sm64huff<<<grid_size, block_size, sm_size,stream0>>>
            (d_sourceData0, d_codewords, d_codewordlens, d_destData0, d_cindex0); 

        vlc_encode_kernel_sm64huff<<<grid_size, block_size, sm_size,stream1>>>
            (d_sourceData1, d_codewords, d_codewordlens, d_destData1, d_cindex1); 
        //testedOK2
                //                d_cw32, d_cw32len, d_cw32idx, 
        //       }
...
//        printf("GPU Encoding time (SM64HUFF): %f (ms)\n", ktime/NT);
        ...
        cudaMemset(d_destDataPacked0, 0, partSize);
//        printf("Num_blocks to be passed to scan is %d.\n", num_scan_elements);
        prescanArray(d_cindex2_0, d_cindex0, num_scan_elements);
...
//        unsigned int *testindex1, *testindex2;
//        testindex1=(uint*)malloc(new_blocks*symbol_type_size);
//        testindex2=(uint*)malloc(new_blocks*symbol_type_size);
//        cudaMemcpy(testindex1,d_cindex0,new_blocks*symbol_type_size,cudaMemcpyDeviceToHost);
//        cudaMemcpy(testindex2,d_cindex2_0,new_blocks*symbol_type_size,cudaMemcpyDeviceToHost);
//
//        printdbg_data_bin("d_cindex.txt", testindex1, new_blocks); 
//        printdbg_data_bin("d_cindex2.txt", testindex2, new_blocks); 
//
//        CUDA_SAFE_CALL(cudaDeviceSynchronize());
        pack2<<< num_scan_elements/16, 16,0,stream0>>>
            ((unsigned int*)d_destData0, d_cindex0, d_cindex2_0, (unsigned int*)d_destDataPacked0, new_num_elements/num_scan_elements);
        CUT_CHECK_ERROR("Pack2 Kernel execution failed\n");
        deallocBlockSums();

        preallocBlockSums(num_scan_elements);
        cudaMemset(d_destDataPacked1, 0, partSize);
        prescanArray(d_cindex2_1, d_cindex1, num_scan_elements);
        pack2<<< num_scan_elements/16, 16,0,stream1>>>
            ((unsigned int*)d_destData1, d_cindex1, d_cindex2_1, (unsigned int*)d_destDataPacked1, new_num_elements/num_scan_elements);
        CUT_CHECK_ERROR("Pack2 Kernel execution failed\n");
        deallocBlockSums();

        CUDA_SAFE_CALL(cudaMemcpyAsync(destData+i, 
                    d_destDataPacked0, 
                    partSize, 
                    cudaMemcpyDeviceToHost,stream0));

        CUDA_SAFE_CALL(cudaMemcpyAsync(destData+i+new_num_elements, 
                    d_destDataPacked1, 
                    partSize, 
    ...
    printdbg_data_bin("gpuoutasync.txt", destData, num_elements); 
...
//    cudaFreeHost(sourceData);
    cudaFreeHost(destData);  	free(codewords);  	free(codewordlens); free(crefData); 

    cudaFreeHost(sourceData);
    //    free(cw32);  free(cw32len); 
    CUDA_SAFE_CALL(cudaFree(d_sourceData0)); 	CUDA_SAFE_CALL(cudaFree(d_destData0)); CUDA_SAFE_CALL(cudaFree(d_destDataPacked0));
    CUDA_SAFE_CALL(cudaFree(d_sourceData1)); 	CUDA_SAFE_CALL(cudaFree(d_destData1)); CUDA_SAFE_CALL(cudaFree(d_destDataPacked1));
    ...
    CUDA_SAFE_CALL(cudaFree(d_cindex0)); CUDA_SAFE_CALL(cudaFree(d_cindex2_0));
    CUDA_SAFE_CALL(cudaFree(d_cindex1)); CUDA_SAFE_CALL(cudaFree(d_cindex2_1));
    free(cindex2);

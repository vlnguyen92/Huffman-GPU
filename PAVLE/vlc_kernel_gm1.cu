 ...
 */
#ifndef _VLC_GM1_KERNEL_H_
#define _VLC_GM1_KERNEL_H_

#include "parameters.h"
#include "pabio_kernels_v2.cu"

/* Variable Length Coding with Table Look-up */
__global__ void static vlc_encode_kernel_gm1(unsigned int* data,
	...
	//equivalent to:
	//codeword_part = codeword_part<<(32-startbit-numbits);
	//atomicOr(&out[kc], codeword_part);
	//equivalent to:
	atomicOr(&out[kc], (codeword_part<<(32-startbit-numbits)));
	...
	if (overflow) {  /* Write a second part of the codeword to the location kc+1 starting from the bit position 0 */
		...
		//equivalent to:
		//codeword_part = codeword_part<<(32-startbit-numbits);
		//atomicOr(&out[kc+1], codeword_part);	
		//atomicOr(&out[kc+1], (codeword_part<<(32-startbit-numbits)));

		atomicOr(&out[kc+1], (codeword<<(32-numbits)));
	} 
}

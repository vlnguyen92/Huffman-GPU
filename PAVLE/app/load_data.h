...
inline void loadData(char *file_name, uint *sourceData, uint *codewords, uint *codewordlens, uint num_elements, uint mem_size, double &H){
	if (file_name == NULL) {
		...
		FILE *f = fopen(file_name, "rb");//rb for read binary
		if (!f) { perror(file_name); exit(1); }
		fseek(f, 0, SEEK_SET); //set position indicator associated with stream to a new pos, add third arg to second arg
		fread(sourceData, 1, mem_size, f); //1 for one byte, mem_size for numbytes in f, total bytes = mem_size.
		fclose(f);
		char buf[100];
		strcpy(buf, file_name);
		strcat(buf, "_codetable.txt");
		f = fopen(buf, "rt");//t for explicit text file

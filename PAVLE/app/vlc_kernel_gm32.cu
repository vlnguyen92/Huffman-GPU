...
#ifndef _VLC_GM32_KERNEL_H_
#define _VLC_GM32_KERNEL_H_
...
/* PAVLE CHARACTERISTICS:
   ...
   3. PARALLEL BIT I/O USING GLOBAL-MEMORY ATOMIC OPERATIONS (COMAPTIBLE WITH CUDA1.1+)
...
__global__ static void vlc_encode_kernel_gm32(unsigned int* data,
...
#else
	unsigned int* as	= (unsigned int*) sm; //sm is the shared memory
//separate data input into 8-byte chunks
	unsigned int val32	= data[kn];
	byte[0]	= (unsigned char)(val32>>24);
	byte[1] = (unsigned char)(val32>>16);
	byte[2] = (unsigned char)(val32>>8);
	...
	#pragma unroll 4
	for(unsigned int i=0; i<4;i++) {
		cw[i] = gm_codewords[byte[i]];//load codeword from global mem chunk-by-chunk
		cwlen[i] = gm_codewordlens[byte[i]];
		codeword = (codeword<<cwlen[i]) | cw[i];//compute codeword based on those chunks
		codewordlen+=cwlen[i];
	}
#endif
	...
	if (k == blockDim.x-1) outidx[blockIdx.x] = as[k] + codewordlen;

	/* Write the codes */
	unsigned int kc = blockIdx.x*blockDim.x +(as[k]>>5); 
